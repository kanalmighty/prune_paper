...
class VGG(nn.Module):
    def __init__(self, num_classes, init_weights=True, cfg=None, origin_cfg=None):
        ...
        self.classifier = nn.Sequential(OrderedDict([
            ('linear1', nn.Linear(self.cfg[-2],  num_classes))
        ]))
...
def vgg_16_bn(num_class, cfg=None):
    origin_cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512]
    return VGG(num_class, cfg=cfg, origin_cfg=origin_cfg)


def vgg_19_bn(num_class, cfg=None):
    origin_cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 256, 'M', 512, 512, 512, 512, 'M', 512, 512, 512, 512]
    return VGG(num_class, cfg=cfg, origin_cfg=origin_cfg)
